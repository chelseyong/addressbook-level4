...
If duplicate task is added, Task Book will give an error message: `This task already exists in the task book`
====
...
[NOTE]
====
You are not allowed to complete a task in `0` hours!
====

[TIP]
If it takes more than 1 day to complete the task, it is recommended to set milestones for the task.
Refer to <<Adding a milestone: `add_milestone`>>

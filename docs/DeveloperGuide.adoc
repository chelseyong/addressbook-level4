...
==== Design Considerations
There are a few ways to implement the password encryption into our product. +
...
* **Alternative 2:** Connect student logging session to an online authentication
...
** 3b1. TB shows that task is entered before.
+
Use case ends.
//end::addTask[]
...
. Add a task with title, description, priority and expected number of hours filled.
...
.. Other incorrect add commands: `add` or `add t/ d/ p/ h/` with compulsory fields not filled in
...
* The filename may be incorrect, i.e. not `taskbook.xml`
